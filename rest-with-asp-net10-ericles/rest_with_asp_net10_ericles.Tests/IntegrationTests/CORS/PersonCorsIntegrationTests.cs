...
public class PersonCorsIntegrationTests : IClassFixture<SqlServerFixture>
{
    ...
    private async Task<PersonDTO> CreatePersonWithAllowedOrigin()
    {
    ...
    {
        var created = await CreatePersonWithAllowedOrigin();
    ...
    {
        // Arrange
        var person = await CreatePersonWithAllowedOrigin();

        // Act
        var response = await _httpClient
            .GetAsync(
                $"api/person/v2/{person.Id}",
                TestContext.Current.CancellationToken);
